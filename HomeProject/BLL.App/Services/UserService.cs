...
{
    public class UserService : BaseEntityService<IAppUnitOfWork, IUserRepository, BLLAppDTO.User, DALAppDTO.User>,
        IUserService
    {
        ...
        public async Task<IEnumerable<BLLAppDTO.User>> GetAllWithCocktailsCountAsync(bool noTracking = true)
        {
            return (await ServiceRepository.GetAllWithCocktailsCountAsync(noTracking))
                .Select(a=> Mapper.Map(a))!;
        }

        public async Task<BLLAppDTO.User?> GetOneWithCocktailsCountAsync(Guid id, bool noTracking = true)
        {
